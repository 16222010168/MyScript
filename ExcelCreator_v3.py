...
class ExcelCreator:
    def __init__(self,source,sheet,assets,password):
        self.source = source
        self.assets = assets
        self.password = password
        self.sheet = sheet
        dfInput = pd.read_excel(source,sheet_name=sheet)
    
    def df_maker(self,data,filename,format):
        format = '.'+ format
        file_locat = 'D:/' + time.strftime("%Y%m%d%H%M%S",time.localtime(time.time())) + filename + format
        dfOutput = pd.DataFrame(data)
        dfOutput.to_excel(file_locat,index=False)
        return dfOutput

    def new_service_passwd_data(self,zhanghao):
...
#账户密码
account = {'appuser':'appuser2017','root':'vm@2013','weblogic':'weblogic@2013','loguser':'log@2017'}
#其他信息
